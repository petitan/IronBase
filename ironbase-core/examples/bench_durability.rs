...
fn main() {
    ...
    // Test 1: Safe mode (fsync every insert)
    {
        ...
        println!(
            "Safe mode:  {} inserts in {:?} ({:.0} ops/sec)",
            count,
            elapsed,
    ...
    // Test 2: Batch mode (fsync every 100 inserts)
    {
        ...
        let db =
            DatabaseCore::open_with_durability(&db_path, DurabilityMode::Batch { batch_size: 100 })
                .unwrap();
        ...
        let elapsed = start.elapsed();
        println!(
            "Batch mode: {} inserts in {:?} ({:.0} ops/sec)",
            count,
            elapsed,
            count as f64 / elapsed.as_secs_f64()
        );
    ...
    // Test 3: Unsafe mode (no fsync)
    {
        ...
        let db =
            DatabaseCore::open_with_durability(&db_path, DurabilityMode::unsafe_manual()).unwrap();
        ...
        let elapsed = start.elapsed();
        println!(
            "Unsafe mode: {} inserts in {:?} ({:.0} ops/sec)",
            count,
            elapsed,
            count as f64 / elapsed.as_secs_f64()
        );
